...
public class Helper extends Controller {
	...
	static String vdcNameGen(String username) {
		...
		vdc_generated_name = vdc_generated_name.length() > LENGTH ? vdc_generated_name
				.substring(0, LENGTH) : vdc_generated_name;
	...
	/**
	 * Returns Virtual machine details. After deployment portal database needs
	 * to be updated . But after deployment session user is lost. To make
	 * updation the virtual machine details are needed. Retrieve the deployment
	 * user for the user's enterprise and get the virtual machine details.
	 * 
	 * @param vdc_id
	 * @param vapp_id
	 * @param vm_id
	 ...
	 * @return
	 */
	static VirtualMachine getVMDetails(Integer vdc_id, Integer vapp_id,
			Integer vm_id, Enterprise enterprise) {
		Logger.info("------------------------- INSIDE getVMDetais()----------");
		Logger.info(" getVMDetais() params : vdc_id " + vdc_id + " vapp_id : "
				+ vapp_id + " vm-id : " + vm_id);
		...
		List<MKT_Configuration> mkt_conf = MarketDAO
				.getDeployEnterprise(enterprise_id);
		...
		Logger.info(" DEPLOY  USERNAME + PASSWORD :" + username + "  "
				+ password);
