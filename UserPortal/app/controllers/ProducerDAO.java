...
public class ProducerDAO {
	...
	static List<OfferPurchased> getOffersPurchasedFromUserId(
	...
	static List<Offer> groupByVDC_EnterpriseView(Integer enterpriseID) {
		...
		// Query query =
		// JPA.em().createQuery("select p from sc_offer as p where p.sc_offer_id in ( select s.sc_offer_id from mkt_enterprise_view as s where s.enterprise_id = ?1 ) GROUP BY p.virtualDataCenter_name");
	...
	static List<Offer> getVappListForVDC_EnterpriseView(Integer enterpriseID,
			String vdc_name_param) {
		Query query2 = JPA.em().createNamedQuery(
				"getVappListForVDC_EnterpriseView");
	...
	static List<OfferPurchased> getSubscribedOfferGivenOfferId(
			Integer sc_offer_id) {
		Query query1 = JPA
				.em()
				.createNativeQuery(
